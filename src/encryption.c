...
uint32_t rot_word_left(const uint32_t word, const uint16_t nb) {
	return (word << nb) | (word >> (32 - nb));
}

uint8_t sub_letter(const uint8_t letter) {
	return (letter ^
	        ((letter << 1) | (letter >> 7)) ^
	        ((letter << 2) | (letter >> 6)) ^
	        ((letter << 3) | (letter >> 5)) ^
...
uint8_t inv_sub_letter(const uint8_t letter) {
...
void add_round_key(uint8_t* current_segment, const uint8_t* expanded_key, const uint8_t round) {
	uint64_t* segment64 = (uint64_t *)current_segment;
	const uint64_t* key64 = (uint64_t *)(expanded_key);
...
void multiply_column(uint8_t* current_segment, uint8_t column) {
	...
	new_col[0] = double_col[0] ^ current_col[3] ^ current_col[2] ^ double_col[1] ^ current_col[1];
	/* 2 * a0 + a3 + a2 + 3 * a1 */
	new_col[1] = double_col[1] ^ current_col[0] ^ current_col[3] ^ double_col[2] ^ current_col[2];
	/* 2 * a1 + a0 + a3 + 3 * a2 */
	new_col[2] = double_col[2] ^ current_col[1] ^ current_col[0] ^ double_col[3] ^ current_col[3];
	/* 2 * a2 + a1 + a0 + 3 * a3 */
	new_col[3] = double_col[3] ^ current_col[2] ^ current_col[1] ^ double_col[0] ^ current_col[0];
	/* 2 * a3 + a2 + a1 + 3 * a0 */
	for (uint8_t i = 0; i < 4; i++) {
